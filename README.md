...
| `feed_open` / `feed_update` | **Market-data integrity** — a keeper posts price + order-book depth to a shared on-chain feed the agent **cannot forge**; the Guardian reads *this*, never a caller-supplied number. The feed is now fed **live DeepBook v3 depth** by [`scripts/keeper/`](scripts/keeper/) → [tx `EXr7EPu9…`](https://suiscan.xyz/testnet/tx/EXr7EPu9W9HoEBHVnrnkNx1xaagbcDr4XyuGDeQUPPyi) | [`AXoLFcpi…`](https://suiscan.xyz/testnet/tx/AXoLFcpihLzFLDzdFzRk6QkTbsivXSVCNTdNuE5DZQRW) · [`5yVaWE8n…`](https://suiscan.xyz/testnet/tx/5yVaWE8nvXYeWw6ZBWbnNHk7MXh5cWKxZ8uGqXftzNUL) |
...
[`agent/agent.py`](agent/agent.py) is the agent half of the fund: an **LLM-powered** loop (structured outputs) that reads the on-chain market feed and vault state, reasons about a trade within the policy bounds, and submits an `agent_trade` proposal.

The point of the whole architecture is that this agent is *untrusted*. Whatever the model decides — even a hallucinated or mis-calibrated risk number — the on-chain Guardian re-derives risk from the same feed and freezes the vault on divergence. The agent proposes; the chain disposes.

```bash
export LLM_API_KEY=...                              # never hardcoded
...
trade, 400 random agent moves that never change the vault's NAV, and a 200-entry
ledger whose keccak hash-chain advances on every append. The deployed bytecode is
immutable — these are test-only and add coverage without touching the package.
...
[ PASS ] prop_guardian_never_approves_unsafe      # L2: ~2.4k swept states — never approves over-ceiling / a lowballed risk
[ PASS ] prop_derive_is_monotone                  # L2: risk ↑ with exposure/price, ↓ with depth (can't thin a book to look safe)
